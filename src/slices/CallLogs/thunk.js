...
export const createTicket = createAsyncThunk("CallLogs/createTicket", async (ticketData) => {
  try {
    // Mock API call
    const response = await new Promise((resolve) => setTimeout(() => resolve({ data: ticketData }), 1000));
...
export const showTicketModalWithCallId = (callId) => (dispatch) => {
  const event = new CustomEvent('flagCall', { detail: { callId } });
