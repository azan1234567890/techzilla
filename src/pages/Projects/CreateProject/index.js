...
const CreateProject = () => {
  ...
  const { selectedFiles, uploadedFiles, selectedFileId, error, copySuccess } = useSelector(state => state.Projects);

  useEffect(() => {
    if (uploadedFiles.length > 0) {
      const latestFile = uploadedFiles[0];
      dispatch(setSelectedFileId(latestFile.id));
    }
  ...
  const handleAcceptedFiles = (files) => {
    ...
    if (filteredFiles.length !== files.length) {
      dispatch(setError("Only PDF and Word files are allowed."));
    }
    ...
    try {
      ...
      dispatch(setError("Failed to accept files. Please try again."));
    }
  };
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      dispatch(addUploadedFiles(newUploadedFiles));

      // Check for backend and upload files to server
      if (process.env.REACT_APP_BACKEND_URL) {
        await dispatch(uploadFilesToServer(selectedFiles));
      }
    } catch (e) {
      dispatch(setError("Failed to upload files. Please try again."));
    }
  };
  ...
  const handleDelete = (id) => {
    try {
      ...
      dispatch(setError("Failed to delete the file. Please try again."));
    }
  };
  ...
  const handleCopyURL = async (url) => {
    try {
      ...
      dispatch(setError("Failed to copy URL. Please try again."));
    }
  };
